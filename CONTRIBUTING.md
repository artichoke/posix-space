...
posix-space requires a recent Ruby and [bundler] for development tasks. The
...
posix-space uses [`rake`](Rakefile) as a task runner. You can see the available
tasks by running:
...
rake test                          # Run posix-space unit tests
```

To lint Ruby sources, posix-space uses [RuboCop]. RuboCop runs as part of the
